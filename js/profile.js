...
function openOwnProfile() {
  ...
  ['dashboard','ranking','habits','projects','chatPage','people','communities'].forEach((id) => { document.querySelector(`#${id}`).hidden = true; });
...
async function sendPasswordEmail(){const email=window.AURA_SESSION.user.email;const button=document.querySelector('#changePasswordButton');const lastRequest=Number(localStorage.getItem('aura67_last_password_reset')||0),remaining=Math.ceil((60000-(Date.now()-lastRequest))/1000);if(remaining>0){showToast(`Você poderá solicitar outro e-mail em ${remaining} segundos.`,'error','Aguarde para reenviar');return false;}button.disabled=true;const resetBase=window.AURA_CURRENT_CONFIG?.siteUrl||window.location.origin;const redirectTo=`${resetBase.replace(/\/$/,'')}/redefinir-senha.html`;const{error}=await window.auraSupabase.auth.resetPasswordForEmail(email,{redirectTo});button.disabled=false;if(error){const limited=/rate limit|too many requests|429/i.test(error.message);showToast(limited?'O serviço de e-mail atingiu o limite de segurança. Aguarde um pouco antes de tentar novamente.':error.message,'error',limited?'Limite temporário de e-mails':'Não foi possível enviar');return false;}localStorage.setItem('aura67_last_password_reset',String(Date.now()));document.querySelector('#passwordMaskedEmail').textContent=maskEmail(email);document.querySelector('#openEmailProvider').href=emailProviderUrl(email);document.querySelector('#accountPopover').hidden=true;document.querySelector('#emailCheckBackdrop').hidden=false;document.body.style.overflow='hidden';showToast('Confira sua caixa de entrada e a pasta de spam.','success','E-mail enviado');return true;}
