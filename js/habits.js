...
document.querySelector('#proofSubmit').addEventListener('click',async()=>{const file=document.querySelector('#proofFile').files[0];const message=document.querySelector('#proofMessage');if(!file||!proofAction){showToast('Tire ou escolha uma foto para continuar.','error','Foto necessária');return;}const button=document.querySelector('#proofSubmit');button.disabled=true;message.textContent='';try{const extension=file.name.split('.').pop().toLowerCase();const path=`${window.AURA_SESSION.user.id}/${todaySaoPaulo()}/${proofAction.id}-${Date.now()}.${extension}`;const{error:uploadError}=await window.auraSupabase.storage.from('action-proofs').upload(path,file);if(uploadError)throw uploadError;const{data,error}=await window.auraSupabase.rpc('submit_action_proof',{p_action_id:proofAction.id,p_evidence_path:path,p_note:document.querySelector('#proofNote').value.trim()});if(error)throw error;pendingProofIds.add(proofAction.id);showToast(`Comprovação enviada. ${data.pending_xp} XP aguardando validação.`,'success','Comprovação recebida');renderHabits();setTimeout(closeProofModal,1300);}catch(error){showToast(error.message,'error');}finally{button.disabled=false;}});
