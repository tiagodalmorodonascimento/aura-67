...
function openProofModal(action){proofAction=action;document.querySelector('#proofTitle').textContent=action.title;document.querySelector('#proofDescription').textContent=`Envie uma foto clara que ajude a comprovar: ${action.description}`;document.querySelector('#proofFile').value='';document.querySelector('#proofNote').value='';if(document.querySelector('#communityProofOptIn'))document.querySelector('#communityProofOptIn').checked=false;document.querySelector('#proofPreview').hidden=true;document.querySelector('#proofPlaceholder').hidden=false;document.querySelector('#proofMessage').textContent='';document.querySelector('#proofBackdrop').hidden=false;document.body.style.overflow='hidden';}
...
document.querySelector('#proofSubmit').addEventListener('click',async()=>{
  const file=document.querySelector('#proofFile').files[0];const message=document.querySelector('#proofMessage');
  if(!file||!proofAction){showToast('Tire ou escolha uma foto para continuar.','error','Foto necessária');return;}
  const button=document.querySelector('#proofSubmit');button.disabled=true;message.textContent='';
  try{
    const safeFile=typeof sanitizeCommunityProofFile==='function'?await sanitizeCommunityProofFile(file):file;const path=`${window.AURA_SESSION.user.id}/${todaySaoPaulo()}/${proofAction.id}-${Date.now()}.jpg`;
    const{error:uploadError}=await window.auraSupabase.storage.from('action-proofs').upload(path,safeFile,{contentType:'image/jpeg'});if(uploadError)throw uploadError;
    const communityOptIn=Boolean(document.querySelector('#communityProofOptIn')?.checked);
    const{data,error}=await window.auraSupabase.rpc('submit_action_proof',{p_action_id:proofAction.id,p_evidence_path:path,p_note:document.querySelector('#proofNote').value.trim(),p_community_opt_in:communityOptIn});if(error)throw error;
    pendingProofIds.add(proofAction.id);showToast(communityOptIn?`Comprovação enviada anonimamente à comunidade. ${data.pending_xp} XP aguardando consenso.`:`Comprovação privada enviada. ${data.pending_xp} XP aguardando validação.`,'success','Comprovação recebida');renderHabits();if(typeof loadCommunitySummary==='function')loadCommunitySummary();setTimeout(closeProofModal,1300);
  }catch(error){showToast(/submit_action_proof|schema cache/i.test(error.message)?'Execute a migration 020 no Supabase para ativar a verificação comunitária.':error.message,'error');}finally{button.disabled=false;}
});
